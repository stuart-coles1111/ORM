#' Plots ski data run times
#'
#' Second run against first run times per race
#'
...
#' @export
#'
timeplot <- function(data, point_size = 2, axis_label_size = 12, axis_title_size = 15, legend_text_size = 12){
    ggplot2::ggplot(data = data,  ggplot2::aes(time1,time2)) +
        ggplot2::geom_point( ggplot2::aes(color = race), size = point_size)  +
        ggplot2::xlab("First run time (secs)") +
        ggplot2::ylab("Second run time (secs)") +
