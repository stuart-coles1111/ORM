...
pred <- function(race_df, race_number, beta, model, plot = TRUE, transform = TRUE){
    ...
    #   }
    if(plot == TRUE){
        p_plot <- ggplot(p_df, aes(diff,prob)) +geom_point(col='steelblue')+ xlab('Time to leading racer (secs)') + ylab('Race Win Probability') %>% print
        p_plot %>% print
    }
...
timeplot_paper <- function(data){
    ggplot(data=data,aes(time1,time2)) + geom_point(aes(color=race))  + xlab("First run time (secs)") + ylab("Second run time (secs)") +
        labs(color = "Event")  +geom_abline(intercept=0, slope=1)+
...
totaltimeplot_paper <- function(data){
    ggplot(data=data,aes(time1,time1+time2)) + geom_point(aes(color=race))  + xlab("First run time (secs)") + ylab("Total race time (secs)") +
        labs(color = "Event") + scale_shape_manual(values=1:10) +theme(axis.text=element_text(size=10),
                                                                       axis.title=element_text(size=12))+ guides(shape = guide_legend(override.aes = list(size = 1)))+ theme(legend.title = element_text(size = 8),
                                                                                                                                                                             legend.text = element_text(size = 8))
}
